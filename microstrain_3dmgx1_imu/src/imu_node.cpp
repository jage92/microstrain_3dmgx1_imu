...
#include "microstrain_3dmgx1_interfaces/srv/add_offset.hpp"

//#include "std_msgs/msg/Bool.hpp"

#include "microstrain_3dmgx1_interfaces/msg/euler_stamped.hpp"
#include "microstrain_3dmgx1_interfaces/srv/publish_tf.hpp"
#include "microstrain_3dmgx1_interfaces/srv/stop_calibrate.hpp"
...
public:
  bool flag = true;
  rclcpp::TimerBase::SharedPtr wallTimer; //! ROS2 Wall timer
  rclcpp::TimerBase::SharedPtr calibrateTimer; //! Timer to capture calibration data
  ...
  rclcpp::Publisher<sensor_msgs::msg::Imu>::SharedPtr imu_data_pub_; //! Publisher of IMU stabilized data
  rclcpp::Publisher<sensor_msgs::msg::Imu>::SharedPtr imu_data_no_grab_pub_; //! Publisher of IMU stabilized data without the grabity component
  rclcpp::Publisher<sensor_msgs::msg::Imu>::SharedPtr imu_stab_data_no_grab_pub_; //! Publisher of IMU stabilized data without the grabity component
  ...
  rclcpp::Publisher<sensor_msgs::msg::MagneticField>::SharedPtr mag_pub; //! Publisher of the delta velocity data
  rclcpp::Publisher<sensor_msgs::msg::MagneticField>::SharedPtr raw_mag_pub; //! Publisher of the delta angle data
  rclcpp::Publisher<geometry_msgs::msg::QuaternionStamped>::SharedPtr orientation_pub; //! Publisher of magnetometer data
  rclcpp::Publisher<geometry_msgs::msg::QuaternionStamped>::SharedPtr stab_orientation_pub; //! Publisher of the orientation data
  rclcpp::Publisher<sensor_msgs::msg::MagneticField>::SharedPtr stab_mag_pub; //! Publisher of the magnetometer data stabilized with the IMU complementary filter
  rclcpp::Publisher<microstrain_3dmgx1_interfaces::msg::EulerStamped>::SharedPtr stab_euler_pub; //! //! Publisher of the stab euler angles
  rclcpp::Publisher<microstrain_3dmgx1_interfaces::msg::EulerStamped>::SharedPtr euler_pub; //! //! Publisher of the euler angles
  ...
  string error_status_; //! Message of the error status
  ...
  std::unique_ptr<tf2_ros::TransformBroadcaster> br; //! Transform broadcaster
  std::unique_ptr<tf2_ros::TransformBroadcaster> stab_br; //! Transform broadcaster
...
//   */
  ...
  error_count_(0)
  {
    ...
    max_drift_rate_ = this->get_parameter("max_drift_rate").get_parameter_value().get<double>();

    this->declare_parameter("time_offset", 0.0);
    offset_ = this->get_parameter("time_offset").get_parameter_value().get<double>();

    this->declare_parameter("frame_id","imu");
    frameid_ = this->get_parameter("frame_id").get_parameter_value().get<string>();

    this->declare_parameter("stab_frame_id","imu");
    stab_frameid_ = this->get_parameter("stab_frame_id").get_parameter_value().get<string>();

    this->declare_parameter("base_frame","base");
    base_frame = this->get_parameter("base_frame").get_parameter_value().get<string>();

    this->declare_parameter("publish_tf", true);
    publishTF = this->get_parameter("publish_tf").get_parameter_value().get<bool>();

    this->declare_parameter("publish_stab_tf", true);
    stab_publishTF = this->get_parameter("publish_stab_tf").get_parameter_value().get<bool>();

    this->declare_parameter("magnitudeZ", 333);
    magnitudeZ = this->get_parameter("magnitudeZ").get_parameter_value().get<int>();

    this->declare_parameter("poll_mode", false);
    poll_mode = this->get_parameter("poll_mode").get_parameter_value().get<bool>();

    this->declare_parameter("raw_sensor", true);
    raw_sensor = this->get_parameter("raw_sensor").get_parameter_value().get<bool>();

    if(raw_sensor) {
      cmd_v.push_back(microstrain_3dmgx1_imu::IMU::CMD_RAW_SENSOR);
      imu_data_raw_pub_ =  this->create_publisher<sensor_msgs::msg::Imu>("imu/raw_data", 1);
      raw_mag_pub =  this->create_publisher<sensor_msgs::msg::MagneticField>("raw_mag", 1);
    }

    this->declare_parameter("stab_mag_accel_angrate", true);
    stab_mag_accel_angrate = this->get_parameter("stab_mag_accel_angrate").get_parameter_value().get<bool>();

    if(stab_mag_accel_angrate) {
      cmd_tmp = microstrain_3dmgx1_imu::IMU::CMD_GYRO_VECTOR;
      imu_stab_data_pub_ =  this->create_publisher<sensor_msgs::msg::Imu>("imu/stab_data", 1);
      stab_mag_pub =  this->create_publisher<sensor_msgs::msg::MagneticField>("stab_mag", 1);
    }

    this->declare_parameter("instant_mag_accel_angrate", false);
    instant_mag_accel_angrate = this->get_parameter("instant_mag_accel_angrate").get_parameter_value().get<bool>();
    ...
    this->declare_parameter("stab_quat_orient", false);
    stab_quat_orient = this->get_parameter("stab_quat_orient").get_parameter_value().get<bool>();

    if(stab_quat_orient) {
      cmd_v.push_back(microstrain_3dmgx1_imu::IMU::CMD_GYRO_QUAT);
      stab_orientation_pub =  this->create_publisher<geometry_msgs::msg::QuaternionStamped>("stab_orientation", 1);
    }

    this->declare_parameter("instant_quat_orient", false);
    instant_quat_orient = this->get_parameter("instant_quat_orient").get_parameter_value().get<bool>();

    if(instant_quat_orient) {
      cmd_v.push_back(microstrain_3dmgx1_imu::IMU::CMD_INSTANT_QUAT);
      orientation_pub =  this->create_publisher<geometry_msgs::msg::QuaternionStamped>("orientation", 1);
    }

    this->declare_parameter("stab_orient_mag_accel_angrate", true);
    stab_orient_mag_accel_angrate = this->get_parameter("stab_orient_mag_accel_angrate").get_parameter_value().get<bool>();

    if(stab_orient_mag_accel_angrate) {
      cmd_tmp = microstrain_3dmgx1_imu::IMU::CMD_GYRO_QUAT_VECTOR;
      stab_mag_pub =  this->create_publisher<sensor_msgs::msg::MagneticField>("stab_mag", 1);
      imu_stab_data_pub_ =  this->create_publisher<sensor_msgs::msg::Imu>("imu/stab_data", 1);
      imu_stab_data_no_grab_pub_ =  this->create_publisher<sensor_msgs::msg::Imu>("imu/stab_data_no_grab", 1);
    }

    this->declare_parameter("instant_orient_mag_accel_angrate", true);
    instant_orient_mag_accel_angrate = this->get_parameter("instant_orient_mag_accel_angrate").get_parameter_value().get<bool>();
    ...
    if(instant_orient_mag_accel_angrate) {
      ...
      mag_pub =  this->create_publisher<sensor_msgs::msg::MagneticField>("mag", 1);
      imu_data_pub_ =  this->create_publisher<sensor_msgs::msg::Imu>("imu/data", 1);
      imu_data_no_grab_pub_ =  this->create_publisher<sensor_msgs::msg::Imu>("imu/data_no_grab", 1);
    }

    this->declare_parameter("stab_euler_angles", true);
    stab_euler_angles = this->get_parameter("stab_euler_angles").get_parameter_value().get<bool>();

    if(stab_euler_angles) {
      cmd_v.push_back(microstrain_3dmgx1_imu::IMU::CMD_GYRO_EULER); //
      stab_euler_pub = this->create_publisher<microstrain_3dmgx1_interfaces::msg::EulerStamped>("stab_euler", 1);
    }

    this->declare_parameter("instant_euler_angles", true);
    instant_euler_angles = this->get_parameter("instant_euler_angles").get_parameter_value().get<bool>();

    if(instant_euler_angles) {
      cmd_v.push_back(microstrain_3dmgx1_imu::IMU::CMD_INSTANT_EULER); //
      euler_pub = this->create_publisher<microstrain_3dmgx1_interfaces::msg::EulerStamped>("euler", 1);
    }

    this->declare_parameter("tf_translation",vector<double>({2.0, 2.0, 0.0}));
    tf_translation = this->get_parameter("tf_translation").get_parameter_value().get<vector<double>>();

    this->declare_parameter("stab_tf_translation",vector<double>({1.0, 2.0, 0.0}));
    ...
    this->declare_parameter("ang_vel_cov",vector<double>({1.01850738002291e-05, 0.0,    0.0,
    ...
    this->declare_parameter("lin_acc_cov",vector<double>({7.25143532155076e-05, 0.0,    0.0,
    ...
    this->declare_parameter("lin_acc_cov_no_grab",vector<double>({0.000210987265734, 0.0,    0.0,
    ...
    this->declare_parameter("mag_cov",vector<double>({1.16666432794325e-05, 0.0,    0.0,
    ...
    this->declare_parameter("stab_mag_cov",vector<double>({1.2198e-5, 0.0,    0.0,
                                                           0.0, 4.1285e-6, 0.0,
                                                           0.0,    0.0,    1.1703e-5}));
    stab_mag_cov = this->get_parameter("stab_mag_cov").get_parameter_value().get<vector<double>>();
    ...
    poll_serv = this->create_service<std_srvs::srv::Empty>("poll",std::bind(&ImuNode::poll, this,std::placeholders::_1,std::placeholders::_2));
    continuous_serv = this->create_service<std_srvs::srv::Empty>("continuous",std::bind(&ImuNode::continuous, this,std::placeholders::_1,std::placeholders::_2));
    start_calibrate_serv = this->create_service<std_srvs::srv::Empty>("start_calibrate",std::bind(&ImuNode::start_calibrate, this,std::placeholders::_1,std::placeholders::_2));
    stop_calibrate_serv = this->create_service<microstrain_3dmgx1_interfaces::srv::StopCalibrate>("stop_calibrate", std::bind(&ImuNode::stop_calibrate, this,std::placeholders::_1,std::placeholders::_2));
    ...
    stab_br = std::make_unique<tf2_ros::TransformBroadcaster>(*this);
    ...
    on_shutdown(std::bind(&ImuNode::closeIMU,this));
    wallTimer = this->create_wall_timer(0ms, std::bind(&ImuNode::wallTimerCallback,this));
    calibrateTimer = this->create_wall_timer(0ms,std::bind(&ImuNode::calibrateTimerCallback,this));
   ...
   */
  bool pubishTFCallback(const std::shared_ptr<microstrain_3dmgx1_interfaces::srv::PublishTf::Request> req, const std::shared_ptr<microstrain_3dmgx1_interfaces::srv::PublishTf::Response> res) {
    string cad1,cad2;

    publishTF = req->publish_tf;
    stab_publishTF = req->stab_publish_tf;
    ...
    this->set_parameter(rclcpp::Parameter("publish_tf", publishTF));
    this->set_parameter(rclcpp::Parameter("publish_stab_tf", stab_publishTF));
  ...
  /**
   * @brief start Configure the communication throught serial port. And configure the imu data rate, do the calibration and starts the data streams.
  ...
  void start()
  {
    ...
    while(intentos) {
      ...
      try
      {
        ...
        diagnostic_.broadcast(diagnostic_msgs::msg::DiagnosticStatus::ERROR, e.what());
      }
      try
      {
    ...
    if(!intentos) {
      RCLCPP_ERROR(this->get_logger(),"Imposible to connect with the IMU");
      diagnostic_.broadcast(diagnostic_msgs::msg::DiagnosticStatus::ERROR, "Imposible to connect with the IMU");
      rclcpp::shutdown();
    }
  }
 ...
 /**
  ...
  void stop()
  {
    if(running)
    {
      try
      {
        ...
        error_count_++;
        RCLCPP_ERROR(this->get_logger(),"Exception thrown while stopping IMU. %s", e.what());
        diagnostic_.broadcast(diagnostic_msgs::msg::DiagnosticStatus::ERROR,"Exception thrown while stopping IMU "+string(e.what()));
      }
  ...
  void temperatureTest(diagnostic_updater::DiagnosticStatusWrapper& status)
  {
    ...
    try {
      ...
      status.summary(diagnostic_msgs::msg::DiagnosticStatus::OK, "Temperature test");
    }
    catch(microstrain_3dmgx1_imu::Exception& e) {
      RCLCPP_ERROR(this->get_logger(),"It is not possible to get temperature: %s",e.what());
      status.summary(diagnostic_msgs::msg::DiagnosticStatus::ERROR, "It is not possible to get temperature: "+string(e.what()));
    }
  }
  ...
  {
    if ((imu_data_pub_!=nullptr && imu_data_pub_->get_subscription_count() > 0) ||
        (imu_data_no_grab_pub_!=nullptr && imu_data_no_grab_pub_->get_subscription_count() > 0) ||
        (imu_stab_data_no_grab_pub_!=nullptr && imu_stab_data_no_grab_pub_->get_subscription_count() > 0) ||
        (imu_stab_data_pub_!=nullptr && imu_stab_data_pub_->get_subscription_count() > 0) ||
        (imu_data_raw_pub_!=nullptr && imu_data_raw_pub_->get_subscription_count() > 0) ||
        (mag_pub!=nullptr && mag_pub->get_subscription_count() > 0) ||
        (raw_mag_pub!=nullptr && raw_mag_pub->get_subscription_count() > 0) ||
        (orientation_pub!=nullptr && orientation_pub->get_subscription_count() > 0) ||
        (stab_orientation_pub!=nullptr && stab_orientation_pub->get_subscription_count() > 0 ) ||
        (stab_mag_pub!=nullptr && stab_mag_pub->get_subscription_count() > 0 ) ||
  ...
  void connectTest(diagnostic_updater::DiagnosticStatusWrapper& status)
  {
    try {
      ...
      status.summary(diagnostic_msgs::msg::DiagnosticStatus::ERROR, "It is not possible to connect: "+string(e.what()));
    }
  }
   ...
   * @return Imu message
   */
  sensor_msgs::msg::Imu createImuData(double *accel, double *angrate, tf2::Quaternion q2, uint64_t time, vector<double> ang_vel_cov, vector<double> lin_acc_cov,vector<double> orientation_cov,std::string frameid_,double *angrate_bias)
  {
   ...
   */
  sensor_msgs::msg::Imu createIMUNoGrabityData(uint64_t time, double *angrate, double *accel,tf2::Quaternion q, vector<double> ang_vel_cov, vector<double> lin_acc_cov_no_grab, vector<double> orientation_cov, std::string frameid_, double *angrate_bias,double *linear_accel_bias)
  {
   ...
   */
  sensor_msgs::msg::MagneticField createMagData(double *mag, uint64_t time,vector<double> mag_cov,std::string frameid_)
  {
    ...
    magData.header.stamp = rclcpp::Time(time);
   ...
   * @return
   */
  void getData(microstrain_3dmgx1_imu::IMU::cmd actual_cmd,sensor_msgs::msg::Imu *iD,sensor_msgs::msg::Imu *iDNG)
  {
    uint64_t time=0;
    double accel[3];
    double angrate[3];
    ...
    sensor_msgs::msg::Imu imuData; //! Message data type for the data IMU
    sensor_msgs::msg::Imu imuDataNoGraviy; //! Message data type for the IMU data without the grabity component
    sensor_msgs::msg::MagneticField magData; //! Message type for the IMU magnetic field data
    geometry_msgs::msg::QuaternionStamped quaternion_msg; //! Message type for the orientation data (Quaternion data type)
    microstrain_3dmgx1_interfaces::msg::EulerStamped eulerMsg; //! Message type for the orientation data (Euler Stamped)
...
//          ROS_INFO("Orientation 1: %lf %lf %lf",roll*180/M_PI,pitch*180/M_PI,yaw*180/M_PI);
      ...
      case microstrain_3dmgx1_imu::IMU::CMD_INSTANT_QUAT:
        ...
        quaternion_msg.header.stamp = rclcpp::Time(time);
...
//          ROS_INFO("Orientation 3: %lf %lf %lf",roll*180/M_PI,pitch*180/M_PI,yaw*180/M_PI);
      ...
      case microstrain_3dmgx1_imu::IMU::CMD_INSTANT_EULER:
        ...
        eulerMsg.header.stamp=rclcpp::Time(time);
...
//          ROS_INFO("Orientation 4b: %lf %lf %lf",roll*180/M_PI,pitch*180/M_PI,yaw*180/M_PI);
      ...
      if(localPublishTF && publishTF) {
        transformStamped.header.stamp = rclcpp::Time(time);
      ...
      error_count_++;
      RCLCPP_WARN(this->get_logger(),"Corrupted data, data packet discarted");
      diagnostic_.broadcast(diagnostic_msgs::msg::DiagnosticStatus::WARN,"Corrupted data, data packet discarted: " + string(ce.what()));
    }
    catch (microstrain_3dmgx1_imu::Exception& e) {
      error_count_++;
      ...
      diagnostic_.broadcast(diagnostic_msgs::msg::DiagnosticStatus::WARN,"Problem with IMU reading" + string(e.what()));
    }
    freq_diag_->tick();
  ...
  void gravityTest(diagnostic_updater::DiagnosticStatusWrapper& status)
  {
    ...
    try {
      ...
      RCLCPP_ERROR(this->get_logger(),"%s", e.what());
      status.summary(diagnostic_msgs::msg::DiagnosticStatus::ERROR, "Could not be done the gravity test: "+string(e.what()));
    }
  }
 ...
 if (running)
      ...
      status.summary(diagnostic_msgs::msg::DiagnosticStatus::ERROR, "IMU is stopped");
  ...
  void biasStatus(diagnostic_updater::DiagnosticStatusWrapper& status)
  {
    if (biasCaptured_)
    {
      status.summary(diagnostic_msgs::msg::DiagnosticStatus::OK, "Bias is captured");
   ...
   */
  bool addOffset(const std::shared_ptr<microstrain_3dmgx1_interfaces::srv::AddOffset::Request> req, const std::shared_ptr<microstrain_3dmgx1_interfaces::srv::AddOffset::Response> resp)
  {
   ...
   * @param event
   */
  void calibrateTimerCallback(){
    double mag[3],min[3],max[3];
    uint64_t time;
    if(calibrationStarted) {
      try {
        imu.collectHardIronData(&time,mag,min,max);
        RCLCPP_INFO(this->get_logger(),"Collecting calibration data: MAG: %lf %lf %lf MIN: %lf %lf %lf MAX: %lf %lf %lf",mag[X],mag[Y],mag[Z],min[X],min[Y],min[Z],max[X],max[Y],max[Z]);
      }
      catch (microstrain_3dmgx1_imu::Exception& e) {
        error_count_++;
        RCLCPP_ERROR(this->get_logger(),"Exception thrown while calibrating IMU %s", e.what());
   ...
   * @return True if the service is running correctly or false if not
   */
  bool stop_calibrate(const std::shared_ptr<microstrain_3dmgx1_interfaces::srv::StopCalibrate::Request> req, const std::shared_ptr<microstrain_3dmgx1_interfaces::srv::StopCalibrate::Response> resp) {
    ...
    if(calibrationStarted) {
      ...
      RCLCPP_INFO(this->get_logger(),"%d",req->calibration_type.type_name.compare("2D"));

      //If the label is present it takes precedence over the id
      if(req->calibration_type.type_name.size()>0) {
        if(!req->calibration_type.type_name.compare("2D") || !req->calibration_type.type_name.compare("2d")) {
          id = microstrain_3dmgx1_interfaces::msg::CalibrationType::CALIBRATION_2D;
        }
        else if(!req->calibration_type.type_name.compare("3D") || !req->calibration_type.type_name.compare("3d")) {
          id = microstrain_3dmgx1_interfaces::msg::CalibrationType::CALIBRATION_3D;
        }
        else
          return false;
      }
      else {
        if(req->calibration_type.type_id == microstrain_3dmgx1_interfaces::msg::CalibrationType::CALIBRATION_2D)
          id = microstrain_3dmgx1_interfaces::msg::CalibrationType::CALIBRATION_2D;
        else if(req->calibration_type.type_id == microstrain_3dmgx1_interfaces::msg::CalibrationType::CALIBRATION_3D)
          id = microstrain_3dmgx1_interfaces::msg::CalibrationType::CALIBRATION_3D;
        else
          return false;
      }
      ...
      try {
        ...
        RCLCPP_INFO(this->get_logger(),"Stopping magnetometer calibration. Offset: %lf %lf %lf",magOffset[X],magOffset[Y],magOffset[Z]);
        calibrated=true;
   ...
   * @return True if the service is running correctly or false if not
   */
  bool calculateBiasCallback(const std::shared_ptr<std_srvs::srv::Empty::Request> &req, const std::shared_ptr<std_srvs::srv::Empty::Response> &resp)
  {
    calculateBias();
    return true;
  }
  ...
  bool poll(const std::shared_ptr<std_srvs::srv::Empty::Request> &req, const std::shared_ptr<std_srvs::srv::Empty::Response> &resp)
  {
    try {
      ...
      diagnostic_.broadcast(diagnostic_msgs::msg::DiagnosticStatus::WARN,"Problem stoping continous mode " + string(e.what()));
    }
  ...
  void captureBias(int it)
  {
    ...
    RCLCPP_INFO(this->get_logger(),"Obtaining biases.");
    diagnostic_.force_update();
    
    //    ROS_INFO("gyro bias: %lf %lf %lf", bias_x_,bias_y_,bias_z_);
    rclcpp::Time start_time = rclcpp::Clock().now();

    while(rclcpp::Clock().now() - start_time < rclcpp::Duration::from_nanoseconds(20.0e9)){
    ...
    RCLCPP_INFO(this->get_logger(),"Accel: %lf %lf %lf",accel[X],accel[Y],accel[Z]);
    linear_accel_bias[X] = accel[X]/count;
    linear_accel_bias[Y] = accel[Y]/count;
    linear_accel_bias[Z] = accel[Z]/count;

    RCLCPP_INFO(this->get_logger(),"Angular velocity: %lf %lf %lf",rate[X],rate[Y],rate[Z]);

    angrate_bias[X] = rate[X]/count;
    angrate_bias[Y] = rate[Y]/count;
    angrate_bias[Z] = rate[Z]/count;

    RCLCPP_INFO(this->get_logger(),"Stab Accel: %lf %lf %lf",stab_accel[X]/ count,stab_accel[Y]/ count,stab_accel[Z]/ count);
    ...
    RCLCPP_INFO(this->get_logger(),"Stab Angular velocity: %lf %lf %lf",stab_rate[X]/count,stab_rate[Y]/count,stab_rate[Z]/count);
    ...
    RCLCPP_INFO(this->get_logger(),"Angular velocidty: %lf %lf %lf",stab_rate[X],stab_rate[Y],stab_rate[Z]);
    ...
    if (/*average_rate < (max_drift_rate_*10) &&*/ stab_average_rate < max_drift_rate_) {
      RCLCPP_INFO(this->get_logger(),"Imu: Capture check succeeded: average angular drift = %f deg/sec, average stab angular drift = %f deg/sec, max drift rate %f deg/se", average_rate*180/M_PI, stab_average_rate*180/M_PI, max_drift_rate_*180/M_PI);
      biasCaptured_ = true;
      RCLCPP_INFO(this->get_logger(),"IMU gyro capture completed.");
      freq_diag_->clear();
    }
    // capture failed
    else{
